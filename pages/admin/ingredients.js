...
// Global ingredients library — with pagination.
...
export default function GlobalIngredientsPage() {
  ...
  function handleUnit(val)   { setUnitFilter(val); reset(); }
            ...
            {[{ unit: 'all', count: stats.total || 0 }, ...topUnits].map(u => (
              <button
                ...
                style={{
                  ...
                  outline: 'none', // fix focus ring bug
                }}
              >
        ...
        {showAdd && (
          <Modal title={editItem ? 'Edit Ingredient' : 'Add Ingredient'} onClose={() => { setShowAdd(false); setEditItem(null); }}>
            <div style={m.field}>
              ...
              <input style={m.input} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Mozzarella" autoFocus onKeyDown={e => e.key === 'Enter' && handleSave()} />
            </div>
            <div style={m.field}>
            ...
            <div style={m.actions}>
              ...
              <button onClick={handleSave} disabled={saving} style={m.saveBtn}>{saving ? 'Saving…' : editItem ? 'Save Changes' : 'Add Ingredient'}</button>
            </div>
          </Modal>
        )}
        ...
        {deleteConfirm && (
          <Modal title="Delete Ingredient" onClose={() => setDeleteConfirm(null)}>
            ...
            <div style={m.actions}>
              ...
              <button onClick={() => handleDelete(deleteConfirm.id)} style={{ ...m.saveBtn, background: 'rgba(232,84,84,0.15)', borderColor: 'rgba(232,84,84,0.3)', color: '#e85454' }}>Delete</button>
            </div>
          </Modal>
        )}
...
const s = {
  ...
  editBtn: { fontSize: 10, fontWeight: 600, padding: '4px 10px', borderRadius: 5, background: 'rgba(2,164,186,0.1)', border: '1px solid rgba(2,164,186,0.25)', color: '#02a4ba', cursor: 'pointer', fontFamily: "'Inter', sans-serif", outline: 'none' },
  deleteBtn: { fontSize: 10, fontWeight: 600, padding: '4px 10px', borderRadius: 5, background: 'rgba(232,84,84,0.08)', border: '1px solid rgba(232,84,84,0.2)', color: '#e85454', cursor: 'pointer', fontFamily: "'Inter', sans-serif", outline: 'none' },
