...
async function extractInvoiceData(imageContents) {
...
// Simple token overlap + substring scoring — good enough for ingredient names
...
function matchScore(a, b) {
  ...
  // Bonus for matching the first meaningful word (usually the protein/product type)
...
async function loadRestaurantIngredients(restaurantId) {
  // Load all ingredients for this restaurant
  ...
  // For each ingredient, find which menu items use it
  // Path: ingredients → component_ingredients → menu_item_components → menu_items
  ...
  // Build a map: ingredient_id → [menu item names]
  const usageMap = {};
...
function matchLineItem(lineItem, restaurantIngredients) {
  ...
  if (top.score >= HIGH_CONFIDENCE_THRESHOLD) {
    // Check if there are other close candidates that might cause confusion
  ...
  // Multiple plausible matches — needs user confirmation
  return {
    status: 'ambiguous',
    matches: scored.slice(0, 5), // top 5 candidates
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Extract invoice data with Claude Vision
    console.log('[parse-invoice] Extracting invoice data...');
    const extracted = await extractInvoiceData(imageContents);
    ...
    const lineItemsWithMatches = (extracted.line_items || []).map((item, idx) => {
      ...
      return {
        _id: `item_${idx}`,
        ...item,
        match_status: matchResult.status,   // 'auto' | 'ambiguous' | 'new'
        match_candidates: matchResult.matches,
        // For 'auto' matches, pre-select the top candidate
