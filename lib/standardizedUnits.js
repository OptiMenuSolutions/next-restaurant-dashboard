 ...
 */
export function calculateStandardizedCost(recipeQuantity, recipeUnit, ingredientCost, ingredientUnit) {
  ...
  const toStandard = convertToStandardUnit(recipeQuantity, recipeUnit);
  const ingredientInStandard = convertToStandardUnit(1, ingredientUnit);

  if (!toStandard.success || !ingredientInStandard.success) {
    // Units incompatible — fall back to simple multiply
