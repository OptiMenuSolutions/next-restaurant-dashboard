...
// ── Launch a new menu ───────────────────────────────────────────────────
// Compares a parsed menu with the restaurant's current dishes. Matching is
// by name, ignoring case and extra spaces. Returns the dishes to keep
// (with their new price/category), the genuinely new dishes (these go to
// review), and the active dishes missing from the new menu (to archive).
...
export async function splitForMenuUpdate(parsedDishes, restaurantId) {
  ...
  if (error) throw new Error(`Could not load your current menu: ${error.message}`);

  // One dish per name; prefer an active dish over an archived one.
  const byName = new Map();
  for (const m of existing || []) {
    const key = normName(m.name);
    const prev = byName.get(key);
    if (!prev || (prev.archived_at && !m.archived_at)) byName.set(key, m);
  }
  ...
  for (const d of parsedDishes) {
    const hit = byName.get(normName(d.name));
    if (!hit) { newDishes.push(d); continue; }
    if (kept.has(hit.id)) continue; // same dish listed twice on the new menu
    kept.add(hit.id);
    matched.push({
      id: hit.id,
      name: hit.name,
      oldPrice: hit.price,
      price: d.price ?? hit.price,
      category: d.category || null,
      description: d.description ?? null,
      restored: !!hit.archived_at,
    });
  }

  const toArchive = (existing || [])
    .filter((m) => !m.archived_at && !kept.has(m.id))
    .map((m) => ({ id: m.id, name: m.name }));

  return { newDishes, matched, toArchive };
}