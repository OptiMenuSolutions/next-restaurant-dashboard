...
import { convertToStandardUnit, getUnitCategory, convertAcrossCategoriesByDensity } from './standardizedUnits';

// Pure quantity converter (unlike calculateStandardizedCost, which is
// cost-oriented and needs a price). Converts fromQty in fromUnit into the
// equivalent quantity in toUnit, via each unit's own standard-unit
// conversion — same underlying tables as calculateStandardizedCost, just
// returning a quantity instead of a dollar figure. Returns null when the
// two units are in incompatible categories (weight vs. volume) or either
// is unrecognized — callers must decide what to do rather than have this
// silently guess.
function convertQuantity(qty, fromUnit, toUnit, ingredientName = '') {
  ...
  // Weight<->volume mismatch — bridge with an estimated density rather
  // than refusing, same reasoning as calculateStandardizedCost: this only
  // fires on lower-cost, small-quantity ingredients in practice.
  if ((fromCategory === 'volume' && toCategory === 'weight') || (fromCategory === 'weight' && toCategory === 'volume')) {
    const bridged = convertAcrossCategoriesByDensity(qty, fromUnit, toCategory, ingredientName);
    const per1 = convertToStandardUnit(1, toUnit, ingredientName);
    if (bridged && per1.success && bridged.unit === per1.unit) return bridged.quantity / per1.quantity;
  }

  const from = convertToStandardUnit(qty, fromUnit, ingredientName);
  const per1 = convertToStandardUnit(1, toUnit, ingredientName);
  if (!from.success || !per1.success || from.unit !== per1.unit) return null;
  return from.quantity / per1.quantity;
}
  ...
  for (const item of (menuItems || [])) {
    ...
    for (const comp of (item.menu_item_components || [])) {
      for (const ci of (comp.component_ingredients || [])) {
        ...
        if (!ingName) continue;
        // Recipe unit (set once, at menu-parse time) and the ingredient's
        // CURRENT tracked unit (which a real invoice can silently change —
        // see confirm-invoice.js standardizing to oz/fl oz) can drift apart.
        // Both are captured here so buildIngredientSalesMap can convert
        // properly instead of assuming they already match.
        ingredients.push({
          name:     ingName,
          nameLower: ingName.toLowerCase(),
          quantity: parseFloat(ci.quantity || 0),
          recipeUnit: ci.unit || ci.ingredients?.unit || '',
          ingredientUnit: ci.ingredients?.unit || ci.unit || '',
        });
      }
    }
...
function buildIngredientSalesMap(posSales, dishIngredientMap) {
  ...
  for (const sale of (posSales || [])) {
    ...
    // Credit each ingredient in the recipe proportionally by its recipe
    // quantity, converted into the ingredient's own tracked unit first —
    // a naive multiply here silently assumed the recipe's unit and the
    // ingredient's current unit already matched, which real invoices can
    // break (confirm-invoice.js can retroactively change an ingredient's
    // unit without ever touching existing recipe rows).
    for (const ing of ingredients) {
      const key = ing.nameLower;
      if (!map[key]) map[key] = {};

      let perSaleQty = ing.quantity;
      if (ing.recipeUnit && ing.ingredientUnit && ing.recipeUnit !== ing.ingredientUnit) {
        const converted = convertQuantity(ing.quantity, ing.recipeUnit, ing.ingredientUnit, ing.name);
        if (converted == null) {
          // Incompatible units (e.g. weight vs. volume) — can't safely
          // convert. Log it rather than silently corrupt consumption
          // tracking with a wrong-by-orders-of-magnitude number; skip
          // crediting this specific sale's consumption for this ingredient.
          console.warn(`[computeWasteRisk] Unit mismatch for "${ing.name}": recipe uses "${ing.recipeUnit}", ingredient tracked in "${ing.ingredientUnit}" — skipping consumption credit for this sale.`);
          continue;
        }
        perSaleQty = converted;
      }

      map[key][saleDate] = (map[key][saleDate] || 0) + (qtySold * perSaleQty);
    }
  }
 ...
 */
export function computeWasteRisk(invoiceItems, invoices, posSales, menuItems, asOfDate = new Date(), freezeSettings = {}) {
  ...
  for (const [name, deliveries] of Object.entries(deliveriesByIngredient)) {
    ...
    for (const delivery of deliveries) {
      const shelfLife = getShelfLife(name, freezeSettings);
      ...
      // FIFO: drain sold quantity against this delivery first — this MUST
      // run for every delivery in order, even ones we're about to skip
      // displaying below. Otherwise an old, expired-and-dropped delivery's
      // share of consumption never gets subtracted, and the running total
      // incorrectly rolls forward to over-drain whichever delivery IS still
      // within the display window (confirmed: this is exactly what zeroed
      // out Ground Beef, Turkey Breast, Ground Pork, and Corned Beef
      // Brisket's current, well-stocked deliveries).
      ...
      // Drop items expired more than EXPIRY_GRACE_DAYS ago — but only from
      // the DISPLAYED list, now that this batch's consumption has already
      // been correctly accounted for above.
