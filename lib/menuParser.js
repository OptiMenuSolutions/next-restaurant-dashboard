...
// Google Vision handles OCR (text extraction from images).
// Claude Haiku handles dish extraction (Pass 1) and recipe building (Pass 2).
...
// ─── Google Vision OCR ────────────────────────────────────────────────────────
// Extracts all text from an image file using Google Cloud Vision API.
// Returns a plain text string of everything visible on the page.

export async function extractTextWithVision(filePath) {
  ...
  if (!text) {
    console.warn(`[vision] No text extracted from ${filePath}`);
  } else {
...
export async function fileToText(file) {
  ...
  if (isPDF) return await pdfToText(file.filepath);
...
// Receives extracted text (string) instead of image contents.

export async function pass1_extractAndClassify(menuText, globalIngredients, restaurantId) {
