...
/**
 * Normalizes an item/dish name for matching across sources (POS exports,
 * AI recommendation titles, menu item names) that may differ in case or
...
/**
 * Finds the best matching key in a name-keyed map for a given (already
 * normalized) lookup name. Tries an exact match first, then falls back to a
 * substring match in either direction — the same fallback pattern
 * PassTicket uses when matching a recommendation title against menu items,
 * applied here so POS sale names and recommendation titles that differ
 * slightly (extra whitespace, a shortened name, a plural) still resolve
 * instead of silently returning "no sales data."
 *
 * @param {object} map          - name(lowercased/trimmed) -> value
 * @param {string} lookupLower  - already-normalized name to search for
 * @returns {string|null}       - the matching key, or null if nothing matched
 */
function findKeyMatch(map, lookupLower) {
  if (!lookupLower) return null;
  if (Object.prototype.hasOwnProperty.call(map, lookupLower)) return lookupLower;
  const keys = Object.keys(map);
  const found = keys.find((k) => k.includes(lookupLower) || lookupLower.includes(k));
  return found || null;
}
...
export function useWeekInReview(restaurantId, wasteRisk, menuItems, rangeFrom, rangeTo) {
  ...
  useEffect(() => {
    if (restaurantId) {
      ...
      // No restaurant to load yet — don't leave callers spinning forever.
      setLoading(false);
    }
  ...
  async function loadWeekData() {
    ...
    try {
      ...
      // Build DOW average map: { itemNameLower: { dow: [qty, ...] } }
      // Keys are normalized (lowercased/trimmed) so a rec title that differs
      // from the POS export only in case or whitespace still matches below.
      const avgByItemDay = {};
      for (const s of (historicSales || [])) {
        const dow = new Date(s.sale_date + 'T12:00:00').getDay();
        const key = normalizeName(s.item_name);
        if (!key) continue;
        if (!avgByItemDay[key])       avgByItemDay[key]      = {};
        if (!avgByItemDay[key][dow])  avgByItemDay[key][dow] = [];
        avgByItemDay[key][dow].push(parseFloat(s.quantity_sold || 0));
      }

      // Build current-week sales map: { date: { itemNameLower: qty } }
      const salesByDateItem = {};
      for (const s of (sales || [])) {
        const key = normalizeName(s.item_name);
        if (!key) continue;
        if (!salesByDateItem[s.sale_date]) salesByDateItem[s.sale_date] = {};
        salesByDateItem[s.sale_date][key] =
          (salesByDateItem[s.sale_date][key] || 0) + parseFloat(s.quantity_sold || 0);
      }
      ...
      const built = days.map(date => {
        ...
        const dishes = dayRecs.slice(0, 3).map((rec, i) => {
          try {
            ...
            // Exact match first, then a substring fallback in either
            // direction — a rec titled "Grilled Branzino" still finds a POS
            // line item named "grilled branzino " or "Branzino" instead of
            // silently resolving to zero sales.
            const soldKey = findKeyMatch(daySales, nameLower);
            const sold    = soldKey ? (daySales[soldKey] ?? 0) : 0;

            const histKey = findKeyMatch(avgByItemDay, nameLower);
            const hist    = histKey ? (avgByItemDay[histKey]?.[dow] || []) : [];
