...
/**
 * OnboardingScreen — 4-step post-signup wizard ported from
 * OptiMenu Onboarding.dc.html: restaurant profile, pass-tag shipping,
 * menu photo upload, invoice/POS connection.
 *
 * Props
 *   onFinish({ name, address, style, cuisine })  called when step 4 completes
...
const POS_LIST = [
  ...
  { key: "upload", label: "I'll upload invoices manually", mono: "PDF" },
];
...
export default function OnboardingScreen({
  ...
  const continueStep = () => {
    ...
    if (step === 4) { onFinish?.({ name, address, style, cuisine }); setDone(true); return; }
    setStep((s) => s + 1);
  };
            ...
            <div style={{ background: "var(--shell)", border: "1px solid var(--line)", borderRadius: 14, boxShadow: "var(--shadow-lg)", padding: "36px 34px" }}>
              ...
              {step === 2 && (
                <div>
                  ...
                  <div style={stepSub}>We ship it same-day or next-day after signup. One passive tag at the pass logs every dish that goes out — no power, no wiring.</div>

                  <div style={{ border: "1px dashed var(--line)", borderRadius: 12, overflow: "hidden", marginBottom: 20, height: 140, background: "var(--panel)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12.5, color: "var(--faint)" }}>
                    Photo: tag mounted at the pass
                  </div>

                  <div style={{ display: "flex", gap: 10, alignItems: "flex-start", marginBottom: 20 }}>
                    <div style={{ width: 20, height: 20, borderRadius: "50%", background: "var(--accent-tint)", color: "var(--accent-deep)", fontSize: 11, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 1 }}>1</div>
                    <span style={{ fontSize: 13.5, color: "var(--muted)", lineHeight: 1.5 }}>Your tag ships to the address you gave us in step 1. We'll walk you through placing and pairing it once it arrives.</span>
                  </div>
              ...
              {step === 4 && (
                <div>
                  ...
                  <div style={stepTitle}>Connect your invoices</div>
                  <div style={stepSub}>We read supplier, date and every line item from what you upload, then re-cost your menu automatically.</div>
                  ...
                  {(posChoice === "upload" || posChoice === null) && (
                    <label style={{ display: "block", border: "1.5px dashed var(--line)", borderRadius: 12, padding: 22, textAlign: "center", background: "var(--panel)", cursor: "pointer" }}>
                      <input type="file" multiple accept="application/pdf,image/*" style={{ display: "none" }} onChange={(e) => onUploadInvoices?.(e.target.files)} />
                      <div style={{ fontFamily: "'IBM Plex Mono',monospace", fontSize: 10.5, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--faint)", marginBottom: 6 }}>Drag files here</div>
                      <div style={{ fontSize: 12, color: "var(--muted)" }}>or upload a PDF, JPG or PNG anytime from Invoices</div>
                    </label>
                  )}
                </div>
              )}
