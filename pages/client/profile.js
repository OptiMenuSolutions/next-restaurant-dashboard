...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .pr-back { display: flex; align-items: center; gap: 6px; font-size: clamp(11px,.85vw,14px); color: '#02a4ba'; background: none; border: none; cursor: pointer; font-family: 'Inter', sans-serif; color: #02a4ba; padding: 0; }
  ...
  /* PLAN BADGE */
  .pr-plan { display: flex; align-items: center; justify-content: space-between; background: #13120f; border: 1px solid rgba(2,164,186,.25); border-radius: 10px; padding: clamp(12px,1.2vw,18px); margin-bottom: clamp(14px,1.4vw,20px); }
  .pr-plan-name { font-size: clamp(13px,1vw,16px); font-weight: 600; color: #e8e2d8; }
  .pr-plan-sub { font-size: clamp(10px,.75vw,12px); color: #4a453e; margin-top: 3px; }
  .pr-plan-price { font-family: 'Playfair Display', serif; font-size: clamp(20px,1.8vw,28px); color: #02a4ba; line-height: 1; }
  .pr-plan-period { font-size: clamp(9px,.68vw,11px); color: #4a453e; text-align: right; margin-top: 2px; }
  ...
  .pr-cancel-btn { background: none; border: 1px solid #2a2620; border-radius: 7px; padding: clamp(8px,.8vw,11px) clamp(12px,1vw,18px); font-size: clamp(12px,.9vw,14px); color: '#4a453e'; cursor: pointer; font-family: 'Inter', sans-serif; white-space: nowrap; transition: all .15s; flex-shrink: 0; color: #4a453e; }
  .pr-cancel-btn:hover { color: #e8e2d8; border-color: #3a3630; }
  .pr-success { font-size: clamp(10px,.75vw,12px); color: #2a8a5a; margin-top: 5px; }
  .pr-error { font-size: clamp(10px,.75vw,12px); color: #c04040; margin-top: 5px; }
  ...
  .pr-link-arrow { font-size: 14px; color: #4a453e; }
  ...
  /* TABS */
  .pr-tabs { display: flex; background: #13120f; border-bottom: 1px solid #2a2620; padding: 0 clamp(16px,2vw,32px); margin-bottom: clamp(14px,1.4vw,20px); position: sticky; top: clamp(36px,4vh,52px); z-index: 5; }
  .pr-tab { padding: clamp(10px,1vh,14px) clamp(12px,1.2vw,18px); font-size: clamp(11px,.85vw,14px); color: #4a453e; border: none; background: none; cursor: pointer; font-family: 'Inter', sans-serif; border-bottom: 2px solid transparent; margin-bottom: -1px; transition: all .15s; }
  .pr-tab.active { color: #02a4ba; border-bottom-color: #02a4ba; }
  .pr-tab:hover:not(.active) { color: '#9a9086'; color: #9a9086; }
`;
...
function Toggle({ on, onChange }) {
  return (
    <button className={`pr-toggle${on ? ' on' : ''}`} onClick={() => onChange(!on)}>
...
export default function ProfilePage() {
  ...
  async function init() {
    ...
    const { data: profile } = await supabase.from('profiles').select('full_name, restaurant_id').eq('id', user.id).single();
    if (profile) {
      setUserName(profile.full_name || '');
      setRestaurantId(profile.restaurant_id);
      if (profile.restaurant_id) {
        const { data: rest } = await supabase.from('restaurants').select('name, target_food_cost').eq('id', profile.restaurant_id).single();
        if (rest) {
  ...
  async function saveName() {
    ...
    setSaving(true);
    const { data: { user } } = await supabase.auth.getUser();
    const { error } = await supabase.from('profiles').update({ full_name: tempName.trim() }).eq('id', user.id);
    setSaving(false);
  ...
  async function sendFeedback() {
    ...
    // In production, send to a feedback table or email
    const { data: { user } } = await supabase.auth.getUser();
    await supabase.from('feedback').insert([{ user_id: user?.id, message: feedbackText.trim(), created_at: new Date().toISOString() }]).catch(() => {});
    setFeedbackText('');
          ...
          {activeTab === 'account' && (
            <>
              ...
              <div className="pr-plan">
                <div>
                  <div className="pr-plan-name">Founding Member</div>
                  <div className="pr-plan-sub">Locked in for life · renews monthly</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div className="pr-plan-price">$59</div>
          ...
          {activeTab === 'notifications' && (
            <div className="pr-section">
              ...
              <div className="pr-toggle-row">
                ...
                <Toggle on={notifPriceAlert} onChange={setNotifPriceAlert} />
              </div>
              <div className="pr-toggle-row">
                <div>
                ...
                <Toggle on={notifLowMargin} onChange={setNotifLowMargin} />
              </div>
            </div>
          )}
