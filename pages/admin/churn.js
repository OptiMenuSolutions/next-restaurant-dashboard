...
export default function ChurnPage() {
  ...
  const filtered = restaurants.filter(r => tierFilter === 'all' || r.tier === tierFilter);
  const { page, setPage, pageItems, totalPages, reset } = usePagination(filtered, PAGE_SIZE);

  function handleFilter(val) { setTierFilter(val); reset(); }
      ...
      <div style={s.page}>
        ...
        <div style={s.filterGroup}>
          {[
            ...
            <FilterButton key={f.value} active={tierFilter === f.value} onClick={() => handleFilter(f.value)}>
              {f.label}
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', maxWidth: 280, justifyContent: 'flex-end' }}>
                        {r.signals.slice(0, 2).map((sig, i) => (
                          <span key={i} style={{ fontSize: 8, fontWeight: 600, padding: '2px 6px', borderRadius: 10, background: '#1a1c23', border: '1px solid #1e2028', color: '#5a6080' }}>
                            {sig.label}
                          </span>
                        ))}
                        {r.signals.length > 2 && <span style={{ fontSize: 8, color: '#3a3e50' }}>+{r.signals.length - 2} more</span>}
                      </div>
                      ...
                      <div style={{ borderTop: '1px solid #1a1c23', padding: '12px 16px', display: 'flex', gap: 24, flexWrap: 'wrap' }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                            {r.signals.map((sig, i) => (
                              <span key={i} style={{ fontSize: 9, fontWeight: 600, padding: '3px 8px', borderRadius: 10, background: `${tc.text}15`, border: `1px solid ${tc.text}40`, color: tc.text }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flexShrink: 0 }}>
                          <button onClick={() => router.push(`/admin/restaurants/${r.id}`)} style={s.actionBtn}>View Profile →</button>
                          {r.owner_email && (
                            <a href={`mailto:${r.owner_email}?subject=Your OptiMenu account&body=Hi ${r.owner_name || 'there'},`}
